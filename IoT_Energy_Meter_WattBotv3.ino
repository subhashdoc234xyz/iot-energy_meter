...
<div class="header">
  ...
  // Live readings
  html += "<div class='grid'>";
  html += "<div class='card'><div class='val' id='dv_voltage'>"   + String(voltage,1)     + "</div><div class='unit'>V</div><div class='lbl'>Voltage</div></div>";
  html += "<div class='card'><div class='val' id='dv_current'>"   + String(current,2)     + "</div><div class='unit'>A</div><div class='lbl'>Current</div></div>";
  html += "<div class='card'><div class='val' id='dv_power'>"     + String(power,1)       + "</div><div class='unit'>W</div><div class='lbl'>Power</div></div>";
  html += "<div class='card'><div class='val' id='dv_energy'>"    + String(energy,3)      + "</div><div class='unit'>kWh</div><div class='lbl'>Energy</div></div>";
  html += "<div class='card'><div class='val' id='dv_frequency'>" + String(frequency,1)   + "</div><div class='unit'>Hz</div><div class='lbl'>Frequency</div></div>";
  html += "<div class='card'><div class='val' id='dv_pf'>"        + String(powerFactor,2) + "</div><div class='unit'>PF</div><div class='lbl'>Power Factor</div></div>";
  html += "</div>";
  ...
  html += "<div class='status' id='statusBar'>Sensor: <span class='" + String(sensorOk ? "badge-ok" : "badge-err") + "'>" + String(sensorOk ? "OK" : "ERROR") + "</span>";
  ...
  // ── Light AI Agent — Floating Bubble + Chat Panel ──
  html += R"rawliteral(

  <!-- Floating bubble -->
  <div id="lightBubble" onclick="toggleLight()" title="Ask Light AI">
    <div id="lightRing"></div>
    <span>&#9889;</span>
    <div id="lightBadge" style="display:none">!</div>
  </div>

  <!-- Chat panel -->
  <div id="lightPanel">
    <div id="lightPanelHeader">
      <div style="display:flex;align-items:center;gap:10px;">
        <div id="lightHeaderIcon">&#9889;</div>
        <div>
          <div id="lightName">Light</div>
          <div id="lightStatus">AI Energy Assistant</div>
        </div>
      </div>
      <div style="display:flex;gap:8px;align-items:center;">
        <button id="lightClearBtn" onclick="clearChat()" title="Clear chat">&#128465;</button>
        <button id="lightCloseBtn" onclick="toggleLight()">&#10005;</button>
      </div>
    </div>
    <div id="lightMessages">
      <div class="lmsg lmsg-ai">
        <div class="lmsg-bubble">&#9889; Hi! I'm <b>Light</b>. Ask me anything about your energy usage, appliances, or electricity bills!</div>
      </div>
    </div>
    <div id="lightInputRow">
      <input id="lightQ" type="text" placeholder="Ask Light..." maxlength="300" onkeydown="if(event.key==='Enter')askLight();">
      <button id="lightSend" onclick="askLight()">&#9654;</button>
    </div>
  </div>
  ...
  #lightBubble {
    ...
    background:#111; border:2px solid rgba(250,204,21,0.7);
    display:flex; align-items:center; justify-content:center;
    font-size:1.6rem; cursor:pointer;
    box-shadow:0 4px 24px rgba(0,0,0,0.5), 0 0 16px rgba(250,204,21,0.3);
    animation:heartbeat 2s ease-in-out infinite;
    transition:border-color 0.2s;
    user-select:none;
  }
  #lightBubble:hover { border-color:#facc15; box-shadow:0 4px 28px rgba(250,204,21,0.5); }
  ...
  #lightPanel {
    ...
    box-shadow:0 16px 48px rgba(0,0,0,0.7), 0 0 24px rgba(250,204,21,0.1);
    backdrop-filter:blur(24px);
    flex-direction:column;
    overflow:hidden;
    animation:slideUp 0.25s ease;
  }
  #lightPanel.open { display:flex; }
  ...
  #lightHeaderIcon {
    ...
    background:#111; border:1.5px solid rgba(250,204,21,0.6);
    display:flex; align-items:center; justify-content:center;
    font-size:1.1rem;
    animation:heartbeat 2s ease-in-out infinite;
  }
  #lightName { font-size:1rem; font-weight:800; color:#fde68a; }
  #lightStatus { font-size:0.7rem; color:#78716c; font-weight:600; letter-spacing:1px; }

  #lightCloseBtn, #lightClearBtn {
    background:none; border:none; color:#78716c;
    cursor:pointer; font-size:1rem; padding:4px 6px;
    border-radius:8px; transition:all 0.15s;
  }
  #lightCloseBtn:hover { color:#ef4444; background:rgba(239,68,68,0.1); }
  #lightClearBtn:hover { color:#94a3b8; background:rgba(148,163,184,0.1); }
  ...
  #lightMessages::-webkit-scrollbar { width:4px; }
  #lightMessages::-webkit-scrollbar-thumb { background:rgba(250,204,21,0.2); border-radius:2px; }

  .lmsg { display:flex; animation:msgIn 0.2s ease; }
  .lmsg-user { justify-content:flex-end; }
  .lmsg-ai   { justify-content:flex-start; }
  ...
  .lmsg-user .lmsg-bubble {
    ...
    border:1px solid rgba(250,204,21,0.3);
    color:#fef9c3; border-bottom-right-radius:4px;
  }
  .lmsg-ai .lmsg-bubble {
  ...
  .lmsg-typing span {
    ...
    background:#facc15; margin:0 2px;
  ...
  #lightQ {
    ...
    background:rgba(15,23,42,0.8); color:#fef9c3;
    font-size:0.9rem; outline:none; transition:border-color 0.2s;
  }
  #lightQ:focus { border-color:#facc15; }
  #lightQ::placeholder { color:#44403c; }
  #lightSend {
  ...
  #lightSend:hover { background:rgba(250,204,21,0.35); }
  #lightSend:disabled { opacity:0.4; cursor:not-allowed; }
  </style>

  <script>
  // ── Chat persistence (survives JS-based refresh) ──
  var LC_KEY = 'lightChat';
  var chatHistory = [];

  function saveChatToStorage() {
    try { localStorage.setItem(LC_KEY, JSON.stringify(chatHistory)); } catch(e){}
  }
  ...
  function renderStoredMessages() {
    ...
    var box = document.getElementById('lightMessages');
    // keep welcome bubble, append stored
  ...
  function appendBubble(role, text, save) {
    var box = document.getElementById('lightMessages');
    var div = document.createElement('div');
    div.className = 'lmsg lmsg-' + role;
    var bub = document.createElement('div');
    bub.className = 'lmsg-bubble';
  ...
  function showTyping() {
    ...
    div.id = 'lightTyping';
    div.innerHTML = '<div class="lmsg-bubble lmsg-typing"><span></span><span></span><span></span></div>';
    box.appendChild(div);
    box.scrollTop = box.scrollHeight;
  }
  function hideTyping() {
    var t = document.getElementById('lightTyping');
    if (t) t.remove();
  }

  function toggleLight() {
    var panel = document.getElementById('lightPanel');
    panel.classList.toggle('open');
    if (panel.classList.contains('open')) {
      document.getElementById('lightBadge').style.display = 'none';
      setTimeout(function(){ document.getElementById('lightQ').focus(); }, 200);
      var box = document.getElementById('lightMessages');
      box.scrollTop = box.scrollHeight;
    }
  }
  ...
  function clearChat() {
    ...
    saveChatToStorage();
    var box = document.getElementById('lightMessages');
    box.innerHTML = '<div class="lmsg lmsg-ai"><div class="lmsg-bubble">&#9889; Chat cleared! Ask me anything about your energy usage.</div></div>';
  }

  function askLight() {
    var inp = document.getElementById('lightQ');
    var q = inp.value.trim();
    if (!q) return;
    inp.value = '';
    appendBubble('user', q, true);
    showTyping();
    document.getElementById('lightSend').disabled = true;
    ...
    .then(function(d){
      hideTyping();
      var txt = d.answer || ('&#9888; ' + (d.error || 'Unknown error'));
      appendBubble('ai', txt, true);
      document.getElementById('lightSend').disabled = false;
    })
    .catch(function(e){
      hideTyping();
      appendBubble('ai', '&#9888; Could not reach server.', true);
      document.getElementById('lightSend').disabled = false;
    });
  }

  // ── JS-based auto-refresh (replaces meta refresh — keeps chat alive) ──
  function refreshDashboard() {
    fetch('/data')
    .then(function(r){ return r.json(); })
    .then(function(d){
      // Update all live values safely
      function setText(id, val) {
        var el = document.getElementById(id);
        if (el) el.textContent = val;
      }
      setText('dv_voltage',     (d.voltage||0).toFixed(1));
      setText('dv_current',     (d.current||0).toFixed(2));
      setText('dv_power',       (d.power||0).toFixed(1));
      setText('dv_energy',      (d.energy||0).toFixed(3));
      setText('dv_frequency',   (d.frequency||0).toFixed(1));
      setText('dv_pf',          (d.power_factor||0).toFixed(2));
    })
    .catch(function(){});
  }
...
void setup() {
  ...
  // ── Light AI Agent proxy ──
  server.on("/ask", HTTP_POST, [](AsyncWebServerRequest* req){}, nullptr,
    [](AsyncWebServerRequest* req, uint8_t* data, size_t len, size_t index, size_t total) {
      if (WiFi.status() != WL_CONNECTED) {
        req->send(503, "application/json", "{\"error\":\"WiFi not connected\"}");
        return;
      }
      String body = "";
      ...
      HTTPClient http;
      String url = String("http://") + PC_IP + ":" + LOG_PORT + "/ask";
      http.begin(url);
      http.addHeader("Content-Type", "application/json");
      http.setTimeout(65000);  // Ollama can be slow
      int code = http.POST(body);
      String resp = (code > 0) ? http.getString() : "{\"error\":\"PC server unreachable (HTTP " + String(code) + ")\"}";
      http.end();
